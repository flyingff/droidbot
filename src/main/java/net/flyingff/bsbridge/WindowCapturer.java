...
public class WindowCapturer implements IPulseSupplier{
	...
	public WindowCapturer(String className, String title, String subWinTitle) {
		System.out.println("Log. Waiting for window ready...");
		waitForWindow(className, title, subWinTitle);
	...
	private final void prepare() {
		...
		System.out.println(String.format("wnd rect: %d,%d,%d,%d", rect.top, rect.right, rect.left, rect.bottom));
	...
	public static void waitForWindow(String clazz, String title, String subWinTitle) {
		HWND handle = u32.FindWindow(clazz, title);
		
		while(true) {
			try {
				Thread.sleep(500);
			} catch (InterruptedException e) { }
			handle = u32.FindWindow(clazz, title);
			if(handle == null || WinBase.INVALID_HANDLE_VALUE.equals(handle)) {
				continue;
			}
			break;
		}
		HWND[] target = new HWND[1];
		if(subWinTitle != null) {
			while(target[0] == null || WinBase.INVALID_HANDLE_VALUE.equals(target[0])) {
				char[] arr = new char[256];
				u32.EnumChildWindows(handle, (hchild, p)->{
					int len = u32.GetWindowText(hchild, arr, 255);
					String text = new String(arr, 0, len);
					if(subWinTitle.equals(text)) {
						target[0] = hchild;
